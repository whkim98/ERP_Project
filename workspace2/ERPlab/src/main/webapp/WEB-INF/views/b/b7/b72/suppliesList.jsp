...
function surf(v, code){      // list ajax 함수 > A4Controller, a4.xml(investmentList select문)
   ...
   var url = "${pageContext.request.contextPath}/supplies/ajax";   // controller mapping
   if(v == ''){
      type = null;
      v = null;
   }
...
function getlist(){
   if(xhr.readyState==4 && xhr.status==200) {   
      ...
      procode.innerHTML += '<tr><td>비품명</td><td>팀명</td><td>구매일자</td><td>합계</td></tr>';
      if(data != ""){
         var data2 = JSON.parse(data);
         data2.forEach(function(map){      // 받아온 list 테이블에 입히기
            newTr = document.createElement("tr");
            newTr.setAttribute("onclick", "selectForm("+map.supplies_no+")");
            procode.appendChild(newTr);
            newTd = document.createElement("td");
            newTd.innerHTML = map.supplies_name;
            newTr.appendChild(newTd);
            newTd = document.createElement("td");
            newTd.innerHTML = map.team_name;
            newTr.appendChild(newTd);
            newTd = document.createElement("td");
            newTd.innerHTML = map.supplies_date;
            newTr.appendChild(newTd);
            newTd = document.createElement("td");
            newTd.innerHTML = map.supplies_total;
            newTr.appendChild(newTd);
         });
      }else {
...
function teamname(){
   ...
   <div class="notosanskr">
      <div align="center">
         <h1 style="font-size: 20pt;">비품관리</h1>
      </div>
      <div class="divform2">
         <div>
            <table>
               <tr>
                  <td>
                     <select name="type">
                        ...
                        <option value="supplies_code" ${param.type == 'supplies_code' ? 'selected' : '' }>비품코드</option>
                        <option value="supplies_name" ${param.type == 'supplies_name' ? 'selected' : '' }>비품명</option>
                     </select>
                  </td>
                  <td>
         ...
         <div style="overflow: scroll;">
            <table id="procode">
               ...
               <c:forEach var="vo" items="${list }">
			     <tr onclick="selectForm(${vo.supplies_no})" class="filter" id="filter">
			   		<td>${vo.supplies_name }</td>
					<td>${vo.team_name }</td>
					<td>${vo.supplies_date }</td>
					<td>${vo.supplies_total }</td>
				</tr>
				</c:forEach>
            </c:if>
            <c:if test="${list == null }">
...
<%--          <div align="right">
   ...
   <!-- 리스트 클릭 시 url 데이터 숨기기 위한 form태그 -->   
         <form action="${pageContext.request.contextPath }/supplies/updateForm" id="content" method="post">
            <input type="hidden" name="supplies_no">
         ...
         <c:choose>
            <c:when test="${map != null }">
               <form action="${pageContext.request.contextPath }/supplies/update" method="POST" id="update">
                  ...
                  <div>
                     <label>비품코드</label>
                     <input type="text" name="supplies_code" id="supplies_code" value="${map.supplies_code }">
                  </div>
				  ...
				  <div>
                     <label>비품명</label>
                     <input type="text" name="supplies_name" id="supplies_name" value="${map.supplies_name}">
                  </div>
				  <div>
                     <label>구매일자</label>
                     <input type="date" name="supplies_date" id="supplies_date" value="${map.supplies_date}">
                  </div>
                  <div>
                  	<label>용도</label>
                  	<input type="text" name="supplies_usage" id="supplies_usage" value="${map.supplies_usage }">
                  </div>
                  <div>
                  	<label>단가</label>
                  	<input type="text" name="supplies_price" id="supplies_price" value="${map.supplies_price }">
                  </div>
                  <div>
                  	<label>비품종류</label>
                  	<select name="suppliestype_no">
                  		<c:forEach var="vo" items="${slist }">
                  			<option value="${vo.suppliestype_no }" ${vo.suppliestype_no == map.suppliestype_no ? 'selected' : "" }>
                  				${vo.suppliestype_name }
                  			</option>
                  		</c:forEach>
                  	</select>
                  </div>
                  ...
                  <div align="right">
                     ...
                     <input type="button" value="delete" onclick="deletei('${bs3_no}', '${dbs3_no }', ${map.supplies_no }, '${comcode_code }')">
                  </div>
               </form>
            </c:when>
            ...
            <c:otherwise>
            <form action="${pageContext.request.contextPath }/supplies/insert" method="POST" id="create">
                  <input type="hidden" name="comcode_code" value="${comcode_code }">
                     <h3>기자재 등록 사항</h3>
                  <div>
                     <label>비품코드</label>
                     <input type="text" name="supplies_code" id="supplies_code">
                  </div>
				  ...
				  <div>
				  	<label>비품명</label>
				  	<input type="text" name="supplies_name" id="supplies_name">
				  </div>
				  <div>
				  	<label>구매일자</label>
				  	<input type="date" name="supplies_date" id="supplies_date">
				  </div>
                  <div>
                  	<label>용도</label>
                  	<input type="text" name="supplies_usage" id="supplies_usage">
                  </div>
                  <div>
                  	<label>단가</label>
                  	<input type="text" name="supplies_price" id="supplies_price">
                  </div>
                  <div>
                  	<label>비품종류</label>
                  	<select name="suppliestype_no">
                  		<c:forEach var="vo" items="${slist }">
                  			<option value="${vo.suppliestype_no }">
                  				${vo.suppliestype_name }
                  			</option>
                  		</c:forEach>
                  	</select>
...
//   bs3_no 세팅
   ...
   function submitt(f) {
	   ...
	   var forsales_cino = document.getElementById("forsales_cino").value;
	   var forsales_blno = document.getElementById("forsales_blno").value;
	   var forsales_date = document.getElementById("forsales_date").value;
	   
	   if(forsales_cino == ""){
		   alert('CINO를 입력해 주세요');
		   document.getElementById('forsales_cino').focus();
		   return false;
	   }else if (forsales_blno == "") {
	     alert("BLNO를 입력해 주세요");
	     document.getElementById("forsales_blno").focus();
	     return false; // 폼 제출 방지
	   }else if (forsales_date == "") {
		 alert("날짜를 입력해 주세요");
		 document.getElementById("forsales_date").focus();
		 return false; // 폼 제출 방지
	   }else {
...
function selectForm(no){
   document.getElementsByName("forsales_no")[0].value = no;
