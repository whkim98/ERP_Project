...
function surf(v, code){      // list ajax 함수 > A4Controller, a4.xml(investmentList select문)
   ...
   var url = "${pageContext.request.contextPath}/tools/ajax";   // controller mapping
   if(v == ''){
      type = null;
      v = null;
   }
...
function getlist(){
   if(xhr.readyState==4 && xhr.status==200) {   
      ...
      procode.innerHTML += '<tr><td>기자재명</td><td>팀명</td><td>구매일자</td><td>합계</td><td>사용연한</td></tr>';
      if(data != ""){
         var data2 = JSON.parse(data);
         data2.forEach(function(map){      // 받아온 list 테이블에 입히기
            newTr = document.createElement("tr");
            newTr.setAttribute("onclick", "selectForm("+map.tools_no+")");
            procode.appendChild(newTr);
            newTd = document.createElement("td");
            newTd.innerHTML = map.tools_name;
            newTr.appendChild(newTd);
            newTd = document.createElement("td");
            newTd.innerHTML = map.team_name;
            newTr.appendChild(newTd);
            newTd = document.createElement("td");
            newTd.innerHTML = map.tools_date;
            newTr.appendChild(newTd);
            newTd = document.createElement("td");
            newTd.innerHTML = map.tools_total;
            newTr.appendChild(newTd);
            newTd = document.createElement("td");
            newTd.innerHTML = map.tools_expiry;
            newTr.appendChild(newTd);
         });
      }else {
      ...
      <div class="divform2">
               ...
               <tr>
                  <td>
                     <select name="type">
                        ...
                        <option value="tools_code" ${param.type == 'tools_code' ? 'selected' : '' }>기자재코드</option>
                        <option value="tools_name" ${param.type == 'tools_name' ? 'selected' : '' }>기자재명</option>
                     </select>
                  </td>
                  <td>
         ...
         <div style="overflow: scroll;">
            <table id="procode">
               ...
               <c:forEach var="vo" items="${list }">
			     <tr onclick="selectForm(${vo.tools_no})" class="filter" id="filter">
			   		<td>${vo.tools_name }</td>
					<td>${vo.team_name }</td>
					<td>${vo.tools_date }</td>
					<td>${vo.tools_total }</td>
					<td>${vo.tools_expiry }</td>
				</tr>
				</c:forEach>
            </c:if>
            <c:if test="${list == null }">
...
<%--          <div align="right">
   ...
   <!-- 리스트 클릭 시 url 데이터 숨기기 위한 form태그 -->   
         <form action="${pageContext.request.contextPath }/tools/updateForm" id="content" method="post">
            <input type="hidden" name="tools_no">
      ...
      <div id="add" class="divform3">
         <c:choose>
            <c:when test="${map != null }">
               <form action="${pageContext.request.contextPath }/tools/update" method="POST" id="update">
                  <input type="hidden" name="comcode_code" value="${comcode_code }">
                  <input type="hidden" name="tools_no" value="${map.tools_no }">
                  ...
                  <div>
                     <label>기자재코드</label>
                     <input type="text" name="tools_code" id="tools_code" value="${map.tools_code }">
                  </div>
                  <div>
                     <label>팀명 </label>
                     <select name="team_no">
                  ...
                  <div>
				    <label>기자재명</label>
					<input type="text" name="tools_name" id="tools_name" value="${map.tools_name }">
				  </div>
				  <div>
                     <label>구매일자 </label>
                     <input type="date" name="tools_date" id="tools_date" value="${map.tools_date }">
                  </div>
                  <div>
				    <label>용도</label>
				    <input type="text" name="tools_usage" id="tools_usage" value="${map.tools_usage }">
				  </div>
				  <div>
				  	<label>사용연한</label>
				  	<input type="date" name="tools_expiry" id="tools_expiry" value="${tools_expiry }">
				  </div>
				  <div>
				  	<label>단가</label>
				  	<input type="text" name="tools_price" id="tools_price" value="${map.tools_price }">
				  </div>
				  <div>
                  	<label>기자재종류</label>
            ...
            <c:otherwise>
            <form action="${pageContext.request.contextPath }/tools/insert" method="POST" id="create">
                  <input type="hidden" name="comcode_code" value="${comcode_code }">
                     <h3>기자재 등록 사항</h3>
                  <div>
                     <label>기자재코드</label>
                     <input type="text" name="tools_code" id="tools_code">
                  </div>
				  ...
				  <div>
				  	<label>기자재명</label>
				  	<input type="text" name="tools_name" id="tools_name">
				  </div>
				  <div>
				  	<label>구매일자</label>
				  	<input type="date" name="tools_date" id="tools_date">
				  </div>
                  <div>
                  	<label>용도</label>
                  	<input type="text" name="tools_usage" id="tools_usage">
                  </div>
                  <div>
                  	<label>사용연한</label>
                  	<input type="date" name="tools_expiry" id="tools_expiry">
                  </div>
                  <div>
                  	<label>단가</label>
                  	<input type="text" name="tools_price" id="tools_price">
                  </div>
                  <div>
                  	<label>기자재종류</label>
                  	<select name="toolstype_no">
                  		<c:forEach var="vo" items="${tlist }">
                  			<option value="${vo.toolstype_no }">
                  				${vo.toolstype_name }
                  			</option>
                  		</c:forEach>
                  	</select>
