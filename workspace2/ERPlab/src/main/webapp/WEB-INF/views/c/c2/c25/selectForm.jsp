...
<div class="notosanskr">
    ...
    <div>
        ...
        <form action="${pageContext.request.contextPath}/c/c2/c25/update">
        	...
        	<input type="hidden" name="localsales_no" id="localsales_no" value="${inmap.localsales_no }">
        	<input type="hidden" name="bs3_no11" id="bs3_no11" value="${inmap.bs3_no1 }">
        	<input type="hidden" name="bs3_no21" id="bs3_no21" value="${inmap.bs3_no2 }">
        ...
        <table>
        	...
        	<tr>
        		...
        		<th>거래처</th>
        		<td><input type="text" name="client_name" id="client_name" value="${inmap.client_name }" onkeyup="searchcl(event, '${comcode_code}')" class="required"></td>
        		<th>사업자등록번호</th>
        	...
        	<tr>
        		<th>결제 조건 / 구분</th>
        		<td colspan="3">
        			<select name="settletype_no" id="settletype_no">
        				<c:forEach var="sl" items="${settleList }">
        					<option value="${sl.settletype_no }" ${sl.settletype_no == inmap.settletype_no ? 'selected' : '' }>${sl.settletype_condition } / ${sl.settletype_type }</option>
        				</c:forEach>
        			</select>
        		</td>
        		<th>대표자</th>
        		<td><input type="text" name="client_representative" id="client_representative" value="${inmap.client_representative }" readonly="readonly"></td>
        		<th>전화번호</th>
        		<td><input type="text" name="client_directno" id="client_directno" value="${inmap.client_directno }" readonly="readonly"></td>
        		<th></th>
        		<td></td>
        	</tr>
        	<tr>
        		...
        		<td colspan="2">
        			<select name="debtor_no" id="debtor_no" onchange="check12()" class="required">
        				<c:forEach var="vo1" items="${dlist }">
        					<option value="${vo1.debtor_no }" id="${vo1.bs3_no }" ${vo1.bs3_no == inmap.bs3_no1 ? 'selected' : '' }>${vo1.bs3_ctgr }</option>
        				</c:forEach>
        			</select>
        		</td>
        		<th colspan="2">대변</th>
        		<td colspan="2">
        			<select name="creditor_no" id="creditor_no" onchange="check22()" class="required">
        				<c:forEach var="vo2" items="${clist }">
        					<option value="${vo2.creditor_no }" id="${vo2.bs3_no }" ${vo2.bs3_no == inmap.bs3_no2 ? 'selected' : '' }>${vo2.bs3_ctgr }</option>
        				</c:forEach>
        			</select>
        		</td>
        	</tr>
	                ...
	                <c:forEach var="i" items="${slist }" varStatus="status">
	                	<tr class="plist">
							...
							<td>
								...
								<input type="hidden" name="sglist1[${status.index }].goodslot_no" id="sglist1[${status.index }].goodslot_no" value="${i.goodslot_no }" readonly="readonly">
								<input type="hidden" name="sglist1[${status.index }].goods_no" id="sglist1[${status.index }].goods_no" value="${i.goods_no }" readonly="readonly">
							</td>
							...
							<td><input type="text" min="0" name="sglist1[${status.index }].salesgoods_qty" id="sglist1[${status.index }].salesgoods_qty" value="${i.salesgoods_qty }" maxlength="4" class="required"></td>
							<td><input type="text" min="0" name="sglist1[${status.index }].salesgoods_price" id="sglist1[${status.index }].salesgoods_price" value="${i.salesgoods_price }" onblur="conculator12(${status.index },this.value)" onkeyup="conculator22(${status.index },event, this.value)" class="required"></td>
							<td><input type="text" name="sglist1[${status.index }].salesgoods_tax" id="sglist1[${status.index }].salesgoods_tax" value="${i.salesgoods_tax }" readonly="readonly"></td>
							<td>
								<input type="text" name="sglist1[${status.index }].salesgoods_total" id="sglist1[${status.index }].salesgoods_total" value="${i.salesgoods_total }" readonly="readonly">
								<input type="hidden" name="sglist1[${status.index }].salesgoods_no" id="sglist1[${status.index }].salesgoods_no" value="${i.salesgoods_no }">
							</td>
							<td>
								<input type="button" onclick="deleteGoods(${i.salesgoods_no},${i.goodslot_no },${i.salesgoods_price },${i.salesgoods_tax },${i.salesgoods_total },'${inmap.localsales_cino }',${i.salesgoods_qty },${i.goods_no })" value="delete">
							</td>
						</tr>
	                </c:forEach>
                </c:if>
    ...
    <div>
        <c:if test="${inmap == null }">
        <form action="${pageContext.request.contextPath}/c/c2/c25/createLocalSales">
        	<input type="hidden" name="comcode_code" id="comcode_code" value="${comcode_code }">
        	<input type="hidden" name="client_no" id="client_no">
        	<input type="hidden" name="team_no" id="team_no" value="12">
        	...
        	<tr>
        		...
        		<td colspan="3">
        			<select name="settletype_no" id="settletype_no">
        				<c:forEach var="sl" items="${settleList }">
        					<option value="${sl.settletype_no }">${sl.settletype_condition } / ${sl.settletype_type }</option>
        				</c:forEach>
        			</select>
        		</td>
        		<th>대표자</th>
                ...
                <tr id="itemTableTitle">
               	...
               	<tr id="procode" class="plist">
					...
					<td>
						...
						<input type="hidden" name="sglist[0].goodslot_no" id="sglist[0].goodslot_no">
						<input type="hidden" name="sglist[0].goods_no" id="sglist[0].goods_no">
					</td>
					...
					<td><input type="text" min="0" name="sglist[0].salesgoods_qty" id="sglist[0].salesgoods_qty" maxlength="4" class="required"></td>
					<td><input type="text" min="0" name="sglist[0].salesgoods_price" id="sglist[0].salesgoods_price" onblur="conculator11(0,this.value)" onkeyup="conculator21(0,event, this.value)" class="required"></td>
					<td><input type="text" name="sglist[0].salesgoods_tax" id="sglist[0].salesgoods_tax" readonly="readonly"></td>
					<td><input type="text" name="sglist[0].salesgoods_total" id="sglist[0].salesgoods_total" readonly="readonly"></td>
					<td>
...
document.getElementById("localsales_date").setAttribute("min", end+"-01-01");
var end = new Date(now_utc-timeOff).toISOString().split("-")[0];
document.getElementById("localsales_date").setAttribute("max", end+"-12-31");
...
function sub(f){
	...
	if(document.getElementById("localsales_cino").value == ""){
		f.localsales_cino.focus();
		return;
		...
		f.creditor_no.focus();
		return;
	}else if(document.getElementById("settletype_no").value == ""){
		alert("결제 구분을 선택해주세요.");
		f.settletype_no.focus();
		return;
	...
	if(document.getElementById("localsales_date").value == ""){
		var ch = confirm("마감일이 등록되지 않았습니다. 등록하시겠습니까?");
		if(ch){
			document.getElementById("localsales_date").value = today;
			f.submit();
		}else {
			return;
		}
    ...
    if (itemTable.rows.length <= 100) {
        ...
        for (let i = 0; i < 15; i++) {
            ...
            if (i === 0) {
                ...
                cells[i].innerHTML = '<td><input type="text" name="crlist['+j+'].goods_name" id="crlist['+j+'].goods_name" readonly="readonly"><input type="hidden" name="sglist['+j+'].goodslot_no" id="sglist['+j+'].goodslot_no"><input type="hidden" name="sglist['+j+'].goods_no" id="sglist['+j+'].goods_no"></td>';
            } else if (i === 2) {
                ...
                cells[i].innerHTML = '<td><input type="text" min="0" name="sglist['+j+'].salesgoods_qty" id="sglist['+j+'].salesgoods_qty" maxlength="4" class="required"></td>';
            } else if (i === 11){
                cells[i].innerHTML = '<td><input type="text" min="0" name="sglist['+j+'].salesgoods_price" id="sglist['+j+'].salesgoods_price" onblur="conculator11('+j+',this.value)" onkeyup="conculator21('+j+',event, this.value)" class="required"></td>';
            } else if (i === 12){
                cells[i].innerHTML = '<td><input type="text" name="sglist['+j+'].salesgoods_tax" id="sglist['+j+'].salesgoods_tax" readonly="readonly"></td>';
            } else if (i === 13){
                cells[i].innerHTML = '<td><input type="text" name="sglist['+j+'].salesgoods_total" id="sglist['+j+'].salesgoods_total" readonly="readonly"></td>';
            } else {
...
var t = 0;
function deleteGoods(no, lno, price, tax, total, code, qty, gno){
	var list = document.getElementById("itemTable");
	...
	var url = "${pageContext.request.contextPath }/c/c2/c25/deleteGoods";
	var param = "salesgoods_no="+encodeURIComponent(no)+"&goodslot_no="+encodeURIComponent(lno)+"&salesgoods_price="+encodeURIComponent(price)+"&salesgoods_tax="+encodeURIComponent(tax)+"&salesgoods_total="+encodeURIComponent(total)+"&localsales_cino="+encodeURIComponent(code)+"&salesgoods_qty="+encodeURIComponent(qty)+"&goods_no="+encodeURIComponent(gno);
...
function deleteGoodscheck(){
	if(xhr.readyState==4 && xhr.status==200) {
		...
		if(data != ""){
			procode.innerHTML = '';
			procode.innerHTML += '<tr><th>조회</th><th>품명</th><th>상품코드</th><th>바코드</th><th>종류</th><th>단위</th><th>규격</th><th>제조사</th><th>책임판매업자</th><th>사입량</th><th>판매 수량</th><th>단가</th><th>세액</th><th>총액</th></tr>';
			...
			data2.forEach(function(map){
				...
				newTd = document.createElement("td");
				newTd.innerHTML = '<td><input type="text" value="'+map.goods_name+'" name="crlist1['+t+'].goods_name" id="crlist1['+t+'].goods_name" readonly="readonly"><input type="hidden" value="'+map.goodslot_no+'" name="sglist1['+t+'].goodslot_no" id="sglist1['+t+'].goodslot_no"><input type="hidden" name="sglist1['+t+'].goods_no" id="sglist1['+t+'].goods_no" value="'+map.goods_no+'" readonly="readonly"></td>';
				newTr.appendChild(newTd);
				newTd = document.createElement("td");
				...
				newTr.appendChild(newTd);
				newTd = document.createElement("td");
				newTd.innerHTML = '<input type="text" min="0" name="sglist1['+t+'].salesgoods_qty" id="sglist1['+t+'].salesgoods_qty" value="'+map.salesgoods_qty+'" maxlength="4" class="required">';
				newTr.appendChild(newTd);
				newTd = document.createElement("td");
				newTd.innerHTML = '<input type="text" min="0" name="sglist1['+t+'].salesgoods_price" id="sglist1['+t+'].salesgoods_price" value="'+map.salesgoods_price+'" onblur="conculator11('+t+',this.value)" onkeyup="conculator21('+t+',event, this.value)" class="required">';
				newTr.appendChild(newTd);
				newTd = document.createElement("td");
				newTd.innerHTML = '<input type="text" name="sglist1['+t+'].salesgoods_tax" id="sglist1['+t+'].salesgoods_tax" value="'+map.salesgoods_tax+'" readonly="readonly">';
				newTr.appendChild(newTd);
				newTd = document.createElement("td");
				newTd.innerHTML = '<input type="text" name="sglist1['+t+'].salesgoods_total" id="sglist1['+t+'].salesgoods_total" value="'+map.salesgoods_total+'" readonly="readonly"><input type="hidden" name="sglist1['+t+'].salesgoods_no" id="sglist1['+t+'].salesgoods_no" value="'+map.salesgoods_no+'">';
				newTr.appendChild(newTd);
				newTd = document.createElement("td");
				newTd.innerHTML = '<input type="button" onclick="deleteGoods('+map.salesgoods_no+','+map.goodslot_no+','+map.salesgoods_price+','+map.salesgoods_tax+','+map.salesgoods_total+',`'+map.salesgoods_code+'`,'+map.salesgoods_qty+','+map.goods_no+')" value="delete">';
				newTr.appendChild(newTd);
				t = Number(t) + 1;
			});
		}else {
			procode.innerHTML = '';
			procode.innerHTML += '<tr><th>품명</th><th>상품코드</th><th>바코드</th><th>종류</th><th>단위</th><th>규격</th><th>제조사</th><th>책임판매업자</th><th>판매수량</th><th>단가</th><th>세액</th><th>총액</th></tr>';
...
function goodsList(code, h){
	let openWin = window.open("${pageContext.request.contextPath}/c/c2/c25/goodsList?comcode_code="+code+"&i="+h, "_blank", "scrollbars=yes, top=150, left=300, width=1000, height=500");
}
function goodsList1(code, h){
	let openWin = window.open("${pageContext.request.contextPath}/c/c2/c25/goodsList1?comcode_code="+code+"&i="+h, "_blank", "scrollbars=yes, top=150, left=300, width=1000, height=500");
}
function searcht(){
...
function clList(code){
	let openWin = window.open("${pageContext.request.contextPath}/c/c2/c25/clList?comcode_code="+code, "_blank", "scrollbars=yes, top=150, left=300, width=300, height=300");
}
...
function conculator11(k,v){
	document.getElementById("sglist["+k+"].salesgoods_tax").value = Math.round(Number(v) * 0.1);
	let tax = document.getElementById("sglist["+k+"].salesgoods_tax").value; 
	document.getElementById("sglist["+k+"].salesgoods_total").value = Math.round(Number(v) + Number(tax));
}
...
function conculator12(k,v){
	document.getElementById("sglist1["+k+"].salesgoods_tax").value = Math.round(Number(v) * 0.1);
	let tax = document.getElementById("sglist1["+k+"].salesgoods_tax").value; 
	document.getElementById("sglist1["+k+"].salesgoods_total").value = Math.round(Number(v) + Number(tax));
}
...
function code(v){
	var url = "${pageContext.request.contextPath }/c/c2/c25/localcino";
	var param = "localsales_code="+encodeURIComponent(v);
	...
	if(xhr.readyState==4 && xhr.status==200) {
		...
		if(data != ""){	
			if(data == "사용 가능한 번호입니다."){
				document.getElementById("cino").innerText = data;
				document.getElementById("cino").style.color = "blue";
				document.getElementById("register").disabled = false;
			}else {
				document.getElementById("cino").innerText = data;
				document.getElementById("cino").style.color = "red";
