...
function surf(v, code){      // list ajax 함수 > A4Controller, a4.xml(investmentList select문)
   ...
   var url = "${pageContext.request.contextPath}/internationsales/export/ajax";   // controller mapping
   if(v == ''){
      type = null;
      v = null;
   }
...
function getlist(){
   if(xhr.readyState==4 && xhr.status==200) {   
      ...
      procode.innerHTML += '<tr><td>CINO</td><td>BLNO</td><td>팀명</td><td>거래처명</td><td>매출일자</td><td>상품명</td></tr>';
      if(data != ""){
         var data2 = JSON.parse(data);
         data2.forEach(function(map){      // 받아온 list 테이블에 입히기
            newTr = document.createElement("tr");
            newTr.setAttribute("onclick", "selectForm("+map.forsales_no+")");
            procode.appendChild(newTr);
            newTd = document.createElement("td");
            newTd.innerHTML = map.forsales_cino;
            newTr.appendChild(newTd);
            newTd = document.createElement("td");
            newTd.innerHTML = map.forsales_blno;
            newTr.appendChild(newTd);
            newTd = document.createElement("td");
            ...
            newTr.appendChild(newTd);
            newTd = document.createElement("td");
            newTd.innerHTML = map.forsales_date;
            newTr.appendChild(newTd);
            newTd = document.createElement("td");
...
function teamname(){
   ...
   <div class="notosanskr">
      ...
      <div class="divform2">
               ...
               <tr>
                  <td>
                     <select name="type">
                        ...
                        <option value="forsales_cino" ${param.type == 'forsales_cino' ? 'selected' : '' }>CINO</option>
                        <option value="forsales_blno" ${param.type == 'forsales_blno' ? 'selected' : '' }>BLNO</option>
                     </select>
                  </td>
                  <td>
         ...
         <div style="overflow: scroll;">
            <table id="procode">
               ...
               <c:forEach var="vo" items="${list }">
			     <tr onclick="selectForm(${vo.forsales_no})" class="filter" id="filter">
         ...
         <div>
		<input type="button" value="ADD" onclick="location.href='${pageContext.request.contextPath}/internationalsales/export?comcode_code=${comcode_code }'">
		</div>
...
<%--          <div align="right">
   ...
   <!-- 리스트 클릭 시 url 데이터 숨기기 위한 form태그 -->   
         <form action="${pageContext.request.contextPath }/internationsales/export/updateForm" id="content" method="post">
            <input type="hidden" name="forsales_no">
      ...
      <div id="add" class="divform3">
         <c:choose>
            <c:when test="${map != null }">
               <form action="${pageContext.request.contextPath }/internationsales/export/update" method="POST" id="update">
                  <input type="hidden" name="comcode_code" value="${comcode_code }">
                  <input type="hidden" name="forsales_no" value="${map.forsales_no }">
                  <input type="hidden" name="prevBlno" value="${prevBlno }">
                  ...
                  <div>
                     <label>CINO</label>
                     <input type="text" name="forsales_cino" id="forsales_cino" value="${map.forsales_cino }" maxlength="30">
                  </div>
                  <div>
                     <label>BLNO </label>
                     <input type="text" name="forsales_blno" id="forsales_blno" value="${map.forsales_blno }" maxlength="30">
                  </div>
				  ...
				  <div>
                     <label>입고예정일 </label>
                     <input type="date" name="forsales_date" id="forsales_date" value="${map.forsales_date }">
                  </div>
                  ...
                  <div>
				    ...
				    <select name="goods_no">
				      <c:forEach var="vo" items="${glist}">
				        <option value="${vo.goods_no}" ${map.goods_no == vo.goods_no ? 'selected' : '' }>${vo.goods_name}</option>
				      </c:forEach>
				    </select>
				  </div>
            ...
            <c:otherwise>
            <form action="${pageContext.request.contextPath }/internationalsales/export/insert" method="POST" id="create">
                  <input type="hidden" name="comcode_code" value="${comcode_code }">
                     <h3>수출관리 등록 사항</h3>
                  <div>
                     <label>CINO </label>
                     <input type="text" name="forsales_cino" id="forsales_cino" maxlength="30">
                  </div>
                  <div>
                     <label>BLNO </label>
                     <input type="text" name="forsales_blno" id="forsales_blno" maxlength="30">
                  </div>
				  ...
				  <div>
				  	<label>매출일자</label>
				  	<input type="date" name="forsales_date" id="forsales_date">
				  </div>
                  ...
                  <div>
				    ...
				    <select name="goods_no">
				      <c:forEach var="vo" items="${glist}">
				        <option value="${vo.goods_no}">${vo.goods_name}</option>
				      </c:forEach>
				    </select>
				  </div>   
                  <div>
