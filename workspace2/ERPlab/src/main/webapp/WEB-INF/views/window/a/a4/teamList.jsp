...
<div>
	...
	<table>
		...
		<c:forEach var="vo" items="${list }">
			<tr onclick="setParentText(${vo.team_no }, '${vo.team_name }', '${vo.team_code }')">
				<td>${vo.team_code}</td>
				<td>${vo.team_name}</td>
			</tr>
		</c:forEach>
...
<script type="text/javascript">
	function setParentText(no, name, code){
    	opener.document.getElementById("team_no").value = no;
    	opener.document.getElementById("team_name").value = name;
    	if(opener.document.getElementById("team_code")){
	    	opener.document.getElementById("team_code").value = code;
    	}
    	window.close();
    }
