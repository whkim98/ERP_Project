...
function setParentText(h, code, barcode, name, gkno, cprice, dcp, cno1, cno2, unit, spec, size, pcg, ea, cname1, cname2, sname, kname){
	...
	opener.document.getElementById("crlist1["+h+"].connectrequest_qty").value = '';
	window.close();
}
		...
		if(xhr.readyState==4 && xhr.status==200) {	
			...
			if(data != "")
				...
				data2.forEach(function(map){
					...
					newTr.setAttribute("onclick", "setParentText('${i}','"+map.goods_code+"','"+map.goods_barcode+"','"+map.goods_name+"',"+map.goodskind_no+","+map.goodslot_price+",'"+map.goods_description+"',"+map.client_no1+","+map.client_no2+",'"+map.goodsst_unit+"','"+map.goodsst_spec+"','"+map.goodsst_size+"','"+map.goodsst_package+"',"+map.goodsst_ea+",'"+map.client_name1+"','"+map.client_name2+"')");
